namespace :test do 
  task :prepare do 
    mkdir_p "GizouTests/GizouTests.xcodeproj/xcshareddata/xcschemes"
    cp Dir.glob('GizouTests/Schemes/*.xcscheme'), "GizouTests/GizouTests.xcodeproj/xcshareddata/xcschemes/"
  end
...
namespace :docs do 
  desc "Generates the Gizou documentation using Appledoc"
...
def apple_doc_command
  "/usr/local/bin/appledoc -o Docs/ -p Gizou -v #{spec_version} -c \"Gizou\" " +
...
def run_tests(scheme, sdk)
  sh("xcodebuild -workspace Gizou.xcworkspace -scheme '#{scheme}' -sdk '#{sdk}' -configuration Release clean test | xcpretty -c ; exit ${PIPESTATUS[0]}") rescue nil
end
